...
public interface AnswerRepository extends Repository<Answer, Long> {
    ...
    @Query("UPDATE Answer a SET a.evaluationState = :next " +
    ...
    default boolean startScoring(Long id) {
        return _updateStateAtomic(id, EvaluationState.SCORING, Set.of(EvaluationState.PENDING, EvaluationState.FAILED)) == 1;
    }

    default boolean completeScoring(Long id) {
        return _updateStateAtomic(id, EvaluationState.SCORED, Set.of(EvaluationState.SCORING)) == 1;
    }

    default boolean failScoring(Long id) {
        return _updateStateAtomic(id, EvaluationState.FAILED, Set.of(EvaluationState.SCORING)) == 1;
    }
}
